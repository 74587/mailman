...
3. 为网关配置安全策略、DNS 策略和用户名路由策略。
4. 创建网关用户，授权可用网关，并配置代理选择、fallback、粘性、限速和会话策略。
...
| 顶级资源 | 网关用户 | 外部客户端连接 Mailman 代理入口时使用的用户名和密码，独立于 Mailman 登录账号。 |
| 网关级资源 | 用户名路由策略 | 只属于某个网关。用户可通过用户名后缀临时切换代理池和策略。 |
...
| 代理网关 | 创建监听端口，进入网关详情后维护概览、用户名路由、安全策略、DNS 策略、网关日志。 |
| 网关用户 | 创建可以登录代理网关的账号，配置授权网关和代理选择策略。 |
| 账号分组 | 单独维护网关用户分组。 |
| 账号标签 | 单独维护网关用户标签。 |
| 网关日志 | 查看访问日志和配置审计。 |
...
用户名路由策略属于具体网关，用于让同一个网关用户通过用户名后缀切换不同代理池、调度算法、安全策略或 DNS 策略。

进入“代理网关 → 选择网关 → 配置策略 → 用户名路由”，点击新增。
...
| GET | `/api/proxy-gateway/route-strategies` | 查询用户名路由策略。 |
| POST | `/api/proxy-gateway/route-strategies` | 创建用户名路由策略。 |
...
| GET | `/api/proxy-gateway/access-logs` | 查询访问日志。 |
...
4. 选择范围：按组/标签。
5. 代理分组：`US Residential`。
6. 代理标签：`login`、`high-quality`。
7. 调度算法：轮询或随机。
8. 粘性策略：按账号，TTL 1800 秒。
9. Fallback：重试换代理，最大重试 2。
10. 最大并发：10。
11. 每分钟连接数：120。
12. 保存。
...
### 15.7 SOCKS5 可以，HTTP CONNECT 不行
